...
# Write the TPM matrix to a CSV file
#write.csv(gene_counts, file = "gene_counts.csv", row.names = TRUE)

```

#Get TPM values and write csv file
...
View(TPM)
# Write the TPM matrix to a CSV file
#write.csv(TPM, file = "TPM_matrix.csv", row.names = TRUE)
...
#perform gene_enrichment on the wtpro list and then the b12pro list.

```{r}
library(clusterProfiler)
library(org.Hs.eg.db)  # Change this to your organism, e.g., org.Mm.eg.db for mouse
library(enrichplot)
library(ggplot2)
library(tidyverse)
...
```{r}
progene <- read.csv("list_wtpro_list_b12pro_RNAseq.csv")
colnames(progene)
```

```{r}
# Convert gene_id column to uppercase
progene$common_wtpro_geneid_RNA <- toupper(progene$common_wtpro_geneid_RNA)
progene$common_b12pro_geneid_RNA <- toupper(progene$common_b12pro_geneid_RNA)
progene$background_geneid_RNA <- toupper(progene$background_geneid_RNA)

```

```{r}

go_enrichment_wtpro <- enrichGO(gene = progene$common_wtpro_geneid_RNA, 
                             OrgDb = org.Hs.eg.db, 
                             keyType = "SYMBOL", 
                             ont = "BP", 
                             pAdjustMethod = "BH",
                            pvalueCutoff = 0.05,
                            qvalueCutoff = 0.2,
                             universe = progene$background_geneid_RNA
                            ) 

```

```{r}
barplot(go_enrichment_wtpro)
```

```{r}

# Convert the enrichGO result to a data frame
go_df <- as.data.frame(go_enrichment_wtpro)

# Filter significant GO terms (adjust p-value < 0.05)
go_df_significant <- go_df %>%
  filter(p.adjust < 0.05)

# Select top 10 significant GO terms (based on adjusted p-value or any other metric)
go_df_top <- go_df_significant %>%
  arrange(p.adjust) %>%
  head(40)

# Create a ggplot bar plot of the top GO terms
ggplot(go_df_top, aes(x = reorder(Description, -p.adjust), y = -log10(p.adjust))) + 
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +  # Flip the axes to make it more readable
  theme_minimal() +
  labs(title = "Top 40 GO Enrichment Terms for WT Pro Genes",
       x = "GO Term",
...
```{r}
go_enrichment_b12pro <- enrichGO(gene = progene$common_b12pro_geneid_RNA, 
                             OrgDb = org.Hs.eg.db, 
                             keyType = "SYMBOL", 
                             ont = "BP", 
                             pAdjustMethod = "BH",
...
```{r}
barplot(go_enrichment_b12pro)
```

```{r}
# Convert the enrichGO result for b12pro to a data frame
go_df_b12pro <- as.data.frame(go_enrichment_b12pro)

# Filter significant GO terms (adjusted p-value < 0.05)
go_df_b12pro_significant <- go_df_b12pro %>%
  filter(p.adjust < 0.05)

# Select top 10 significant GO terms (based on adjusted p-value)
go_df_b12pro_top <- go_df_b12pro_significant %>%
  arrange(p.adjust) %>%
  head(40)

# Create a ggplot bar plot of the top GO terms for b12pro
ggplot(go_df_b12pro_top, aes(x = reorder(Description, -p.adjust), y = -log10(p.adjust))) + 
  geom_bar(stat = "identity", fill = "darkorange") +
  coord_flip() +  # Flip the axes to make it more readable
  theme_minimal() +
  labs(title = "Top 40 GO Enrichment Terms for B12 Pro Genes",
       x = "GO Term",
       y = "-log10 Adjusted P-value") +
  theme(axis.text.y = element_text(size = 10),  # Adjust font size for readability
        plot.title = element_text(hjust = 0.5))  # Center the plot title
...
```{r}
#write.csv(as.data.frame(go_enrichment_wtpro), file = "go_enrichment_allwtpro.csv.06232025")
#write.csv(as.data.frame(go_enrichment_b12pro), file = "go_enrichment_allb12pro.csv.06232025")
